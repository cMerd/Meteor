...
int main() {
  ...
  while (!raylib::WindowShouldClose()) {
    ...
    if (currentMenu == MAIN_MENU) {
        ...
        super_enemy_loop_continue3:
          ...
          for (size_t j = 0; j < p.getBullet().size(); j++) {
            ...
            if (CheckCollisionRecs(enemyHitbox, bulletHitBox)) {
              ...
              goto super_enemy_loop_continue3;
            }
          }
          if (CheckCollisionCircleRec(p.getPos(), 20.0f, enemyHitbox)) {
            if (!forceShield) {
        ...
        smart_enemy_loop_continue3:
          ...
          for (size_t j = 0; j < p.getBullet().size(); j++) {
            ...
            if (CheckCollisionRecs(enemyHitbox, bulletHitBox)) {
              ...
              goto smart_enemy_loop_continue3;
            }
          }
          if (CheckCollisionCircleRec(p.getPos(), 20.0f, enemyHitbox)) {
            if (!forceShield) {
      ...
      DrawText((std::string("You survived for: ") + formatTime(elapsed_time_s))
                   .c_str(),
               30, 100, 50, raylib::WHITE);
      DrawText(coinText.c_str(), 30, 180, 50, raylib::WHITE);
      raylib::DrawTexture(coinIcon,
                          45 + raylib::MeasureText(coinText.c_str(), 50), 165,
                          raylib::WHITE);
